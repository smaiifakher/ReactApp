...
import getColumns from './CountriesColumns';
import Edit from "@material-ui/icons/Edit";
import {Switch} from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteCountries, getCountries} from "../../repo/countriesRepo";
...
const CountriesTable = (props) => {
  ...
  return (<div>
    <MaterialTable
      ...
      data={async (query) => {
        setLoading(true);
        const res = await getCountries(query.page, query.pageSize, query.search);
        setLoading(false);
        return ({
      ...
      actions={[{
        icon: () => <Edit/>, tooltip: 'Edit', onClick: (event, rowData) => {
          history.push({
            pathname: `/countries/update/${rowData.id}`, user: rowData
          })
        }
      }, {
