...
import getColumns from './MarketsColumns';
import Edit from "@material-ui/icons/Edit";
import {Switch} from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMarkets, getMarkets} from "../../repo/marketsRepo";
...
const MarketsTable = (props) => {
  ...
  return (<div>
    <MaterialTable
      ...
      data={async (query) => {
        setLoading(true);
        const res = await getMarkets(query.page, query.pageSize, query.search);
        setLoading(false);
        return ({
      ...
      actions={[{
        icon: () => <Edit/>, tooltip: 'Edit', onClick: (event, rowData) => {
          history.push({
            pathname: `/markets/update/${rowData.id}`, user: rowData
          })
        }
      }, {
...
export default withRouter(MarketsTable);
