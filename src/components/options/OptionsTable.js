...
import getColumns from './OptionsColumns';
import Edit from "@material-ui/icons/Edit";
import {Switch} from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteOptions, getOptions} from "../../repo/optionsRepo";
...
const OptionsTable = (props) => {
  ...
  return (<div>
    <MaterialTable
      ...
      data={async (query) => {
        setLoading(true);
        const res = await getOptions(query.page, query.pageSize, query.search);
        setLoading(false);
        return ({
      ...
      actions={[{
        icon: () => <Edit/>, tooltip: 'Edit', onClick: (event, rowData) => {
          history.push({
            pathname: `/options/update/${rowData.id}`, user: rowData
          })
        }
      }, {
...
export default withRouter(OptionsTable);
