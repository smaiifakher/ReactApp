...
import getColumns from './M_Option_ValuesColumns';
import Edit from "@material-ui/icons/Edit";
import {Switch} from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteM_Option_Values, getM_Option_Values} from "../../repo/m_option_valuesRepo";
import {Loading} from "../templates/Loading";
/*
Documentation on developing the Material-Table can be found at https://material-table.com/
*/

const M_Option_ValuesTable = (props) => {
  ...
  const handleRowDelete = (oldData, resolve) => {
    setLoading(true);
    deleteM_Option_Values(oldData.id)
      .then(res => {
        resolve();
        setLoading(false);
      })
  ...
  return (<div>
    <MaterialTable
      minRows={20}
      title="M_Option_Values Data"
      columns={columns}
      data={async (query) => {
        setLoading(true);
        const res = await getM_Option_Values(query.page, query.pageSize, query.search);
        setLoading(false);
        return ({
      ...
      actions={[{
        icon: () => <Edit/>, tooltip: 'Edit', onClick: (event, rowData) => {
          history.push({
            pathname: `/m_option_values/update/${rowData.id}`, user: rowData
          })
        }
      }, {
        ...
        isFreeAction: true, onClick: (event, rowData) => {
          history.push("/m_option_values/add")
        }
      }]}
...
export default withRouter(M_Option_ValuesTable);
