...
import getColumns from './CategoriesColumns';
import Edit from "@material-ui/icons/Edit";
import {Switch} from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteCategories, getCategories} from "../../repo/categoriesRepo";
...
const CategoriesTable = (props) => {
  ...
  return (<div>
    <MaterialTable
      ...
      data={async (query) => {
        setLoading(true);
        const res = await getCategories(query.page, query.pageSize, query.search);
        setLoading(false);
        return ({
      ...
      actions={[{
        icon: () => <Edit/>, tooltip: 'Edit', onClick: (event, rowData) => {
          history.push({
            pathname: `/categories/update/${rowData.id}`, user: rowData
          })
        }
      }, {
